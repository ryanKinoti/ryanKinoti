# Hey there, I'm Ryan Kinoti 🫡

<div align="center">
  
  **Software Developer | Vibe Coder | Machine Learning Enthusiast | Cybersecurity Learner**
  
  *Building commissioned solutions & exploring the intersection of code, intelligence, creativity and ingenuity*
  
  📍 Nairobi, Kenya
  
</div>

---
## 🚀 About Me

Fresh graduate turned full-stack developer, currently working on commissioned projects that solve real-world business problems. I'm passionate about building scalable applications and diving deep into fun ML implementations and cybersecurity practices.

When I'm not coding, you'll find me optimizing my development setup, exploring the latest in GPU computing, or diving into a good webnovel series.

```typescript
const ryan = {
    location: "Nairobi, Kenya",
    currentFocus: ["Web Development", "ML Recommender Systems", "Cybersecurity"],
    workingOn: ["Laptop Repair Shop System", "Real Estate Management Platform"],
    learning: ["Advanced Security Practices", "Game Development Prep"],
    setup: {
        laptop: "Lenovo Legion 5i (RTX 5060)",
        ides: ["PyCharm", "WebStorm", "PhpStorm", "CLion"],
        terminal: "PowerShell + Windows Terminal"
    },
    funFact: "I treat my IDE configurations with the same care as production code"
};
```

---
## 🛠️ Tech Stack

### Languages
![Python](https://img.shields.io/badge/Python-3776AB?style=for-the-badge&logo=python&logoColor=white)
![JavaScript](https://img.shields.io/badge/JavaScript-F7DF1E?style=for-the-badge&logo=javascript&logoColor=black)
![TypeScript](https://img.shields.io/badge/TypeScript-007ACC?style=for-the-badge&logo=typescript&logoColor=white)

### Frameworks & Libraries
![Next.js](https://img.shields.io/badge/Next.js-000000?style=for-the-badge&logo=next.js&logoColor=white)
![React](https://img.shields.io/badge/React-20232A?style=for-the-badge&logo=react&logoColor=61DAFB)
![Prisma](https://img.shields.io/badge/Prisma-2D3748?style=for-the-badge&logo=prisma&logoColor=white)
![Better-Auth](https://img.shields.io/badge/Better-Auth-000000?style=for-the-badge&logo=react&logoColor=white)
![Auth.js](https://img.shields.io/badge/Auth.js-000000?style=for-the-badge&logo=auth0&logoColor=white)
![Zustand](https://img.shields.io/badge/Zustand-000000?style=for-the-badge&logo=react&logoColor=white)

### Tools & Platforms
![Git](https://img.shields.io/badge/Git-F05032?style=for-the-badge&logo=git&logoColor=white)
![Vercel](https://img.shields.io/badge/Vercel-000000?style=for-the-badge&logo=vercel&logoColor=white)
![Firebase](https://img.shields.io/badge/Firebase-FFCA28?style=for-the-badge&logo=firebase&logoColor=black)
![NVIDIA](https://img.shields.io/badge/NVIDIA-76B900?style=for-the-badge&logo=nvidia&logoColor=white)

### Development Environment
![JetBrains](https://img.shields.io/badge/JetBrains-000000?style=for-the-badge&logo=jetbrains&logoColor=white)
![Windows](https://img.shields.io/badge/Windows_11-0078D6?style=for-the-badge&logo=windows&logoColor=white)
![PowerShell](https://img.shields.io/badge/PowerShell-5391FE?style=for-the-badge&logo=powershell&logoColor=white)

---
## 💼 Current Projects

### 🔧 Laptop Repair Shop Management System
A comprehensive Next.js application featuring role-based access control for staff and customers, built with Prisma and modern authentication practices. Streamlining repair workflows and customer communications.

**Tech:** Next.js, Prisma, TypeScript, Better-Auth

### 🏢 Wiprim Holdings Management System
Real estate management platform handling rent statements and payment reconciliation from telecom providers. Complex data synchronization and financial tracking.

**Tech:** Next.js, Prisma, TypeScript, Better-Auth, Capacitor, Ionic-Framework

---
## 📊 GitHub Stats

<div align="center">
  
  ![Ryan's GitHub Stats](https://github-readme-stats.vercel.app/api?username=ryanKinoti&show_icons=true&theme=tokyonight&hide_border=true&count_private=true)
  
  ![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=ryanKinoti&layout=compact&theme=tokyonight&hide_border=true)
  
  ![GitHub Streak](https://github-readme-streak-stats.herokuapp.com/?user=ryanKinoti&theme=tokyonight&hide_border=true)

</div>

---
## 🤝 Let's Connect

I'm always interested in collaborating on interesting projects, discussing tech, or just connecting with fellow developers!

[![GitHub](https://img.shields.io/badge/GitHub-100000?style=for-the-badge&logo=github&logoColor=white)](https://github.com/ryanKinoti)

---
<div align="center">
  
  ### 💡 "The best error message is the one that never shows up."
  
  ![Profile Views](https://komarev.com/ghpvc/?username=ryanKinoti&color=blueviolet&style=for-the-badge)
  
</div>

---
<sub>⚡ Powered by caffeine, curiosity, and a really good GPU</sub>
